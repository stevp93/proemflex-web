    ...
    return (
        <section className="py-16 sm:py-20 md:py-28" style={{ background: "#111820" }}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    <div className="glass-strong rounded-2xl p-6 sm:p-8 lg:p-10 bg-gradient-to-r from-[#00F2FE]/10 to-transparent">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5 lg:gap-6">
                    {newsArticles.slice(1).map((article, i) => (
                        <motion.article
                            ...
                            className={`group glass rounded-xl p-5 sm:p-6 bg-gradient-to-br ${article.gradient} transition-all duration-300 hover:scale-[1.02] flex flex-col`}
                            style={{ border: `1px solid ${article.accent}15` }}
                        >
                            <div className="flex items-center justify-between mb-3">
                                <span
                                    className="text-[0.6rem] sm:text-xs font-display font-semibold uppercase tracking-[0.15em] px-2.5 py-1 rounded-full"
                                    style={{
                                ...
                                <span className="text-[0.6rem] text-[#4B5563]">{article.date}</span>
                            </div>

                            <h3 className="font-display font-bold text-base sm:text-lg text-white mb-2 tracking-tight group-hover:text-[#00F2FE] transition-colors">
                                {article.title}
                            </h3>
                            ...
                            <div className="flex items-center justify-between border-t border-[rgba(255,255,255,0.04)] pt-3">
                                <span className="text-[0.6rem] text-[#4B5563]">{article.readTime} de lectura</span>
                                <Link
                                    href="/contacto"
                                    className="text-xs text-[#00F2FE] hover:text-white transition-colors font-display font-medium inline-flex items-center gap-1"
                                >
                                    Leer más
                ...
                >
                    <div className="glass-strong rounded-xl sm:rounded-2xl p-6 sm:p-8 max-w-2xl mx-auto">
                        <h3 className="font-display font-bold text-lg sm:text-xl text-white mb-2">
                            ¿Quieres recibir nuestras novedades?
                        </h3>
                        <p className="text-sm text-[#9CA3AF] mb-4">
                            Mantente al día con innovaciones, certificaciones y noticias de PROEMFLEX S.A.S.
                        </p>
                        <Link href="/contacto" className="btn-primary text-sm inline-flex items-center gap-2">
